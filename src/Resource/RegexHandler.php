...
class RegexHandler
{
    ...
    public static function isPin($string)
    {
        ...
        // error messages on failure
        $errorMessage = [
            'alpha' => 'PIN code must contain only numeric characters.',
            'number' => 'PIN code must contain only (' . $MINPASSLENGTH . ') numbers.',
            'length' => 'PIN code must contain only (' . $MINPASSLENGTH . ') numbers.',
        ];
        ...
        try {
            ...
            // ensure string is not empty
            if (is_null($string)) {
                throw new \Exception\RegexPin('Pin must not be empty.');
            }

            // loop through the rules. If any fail, throw an exception
            foreach ($rules as $k => $v) {
                if (!$v) {
                    throw new \Exception\RegexPin($errorMessage[$k] . '<br />');
                }
            }
